...
use super::types::{TradePath, PoolMap, Pool};
...
pub fn optimize_amount_out(required_trade_paths: Vec<TradePath>, pool_map: PoolMap, amount_in:BigUint) ->
(Vec<BigUint>, BigUint) {
    ...
    let optimizer = Optimizer::new(
        sorted_required_paths,
         pool_map, total_amount.clone());
    ...
    for (i, split) in splits.iter().enumerate() {
        ...
        if split > &BigUint::ZERO {
            path_splits.push(format!("Path:{:?} Split %:{:.4}", required_trade_paths[i].tokens, Pool::to_f64(split)))
        }
...
pub fn optimize_amount_in(required_trade_paths: Vec<TradePath>, pool_map: PoolMap, amount_out:BigUint) ->
(Vec<BigUint>, BigUint) {
...
impl Optimizer {
    ...
    fn calculate_output(&self, splits: &[f64]) -> f64 {
        ...
        let active_splits: usize = splits.iter()
            .filter(|&&split| split > 1e-10)
            .count();
        ...
        for (i, &split) in splits.iter().enumerate() {
            ...
            if amount_in > BigUint::zero() {
                let amount_out = self.paths[i].get_amount_out(
                    &amount_in, &mut temp_pools);
    ...
    fn calculate_input(&self, splits: &[f64]) -> f64 {
        ...
        for (i, &split) in splits.iter().enumerate() {
            ...
            if amount_out > BigUint::zero() {
                let amount_in = self.paths[i].get_amount_in(
                    &amount_out, &mut temp_pools);
                let hop_count = self.paths[i].tokens.len()-1;
    ...
    fn optimize(&self) -> (Vec<BigUint>, BigUint, PoolMap) {
        ...
        for iteration in 0..250 {
            ...
            let gradient_norm: f64 = gradient.iter()
                .map(|g| g * g)
                .sum::<f64>()
                .sqrt();
            ...
            let mut new_splits: Vec<f64> = splits.iter()
                .zip(gradient.iter())
            ...
            if new_output > best_output {
                ...
                println!("Iteration {}: output = {}, step_size = {}", 
    ...
    fn optimize_input(&self) -> (Vec<BigUint>, BigUint, PoolMap) {
        ...
        for iteration in 0..250 {
            ...
            if new_input > best_input {
                ...
                println!("NEW BEST ******* Iteration {}: input = {}, step_size = {}", 
                        iteration, 1.0/best_input, step_size);
            } else {
                step_size *= 0.7;  // Reduce step size
